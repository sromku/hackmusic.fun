...
export async function readPortableBackup() {
  ...
  const [events, participants, submissions, reactions, activityEvents, analyticsPageviews] = await Promise.all([
    allRows(`SELECT id, code, title, status, scheduled_for, queue_mode, music_source, current_submission_id,
      host_pin, join_passcode_hash, join_passcode_salt, created_at
      FROM events ORDER BY created_at ASC, id ASC`),
    allRows(`SELECT id, public_id, event_id, display_name, initials, color, score, created_at
    ...
    allRows(`SELECT id, event_id, participant_id, provider_track_id, title, artist, duration,
      color, status, skip_reason, skip_percent, submitted_at
