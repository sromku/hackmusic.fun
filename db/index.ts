...
async function initializePartySchema() {
  ...
  await d1.batch([
    d1.prepare(`CREATE TABLE IF NOT EXISTS events (
      ...
      theme TEXT,
      reveal_pickers INTEGER NOT NULL DEFAULT 0,
    ...
    d1.prepare(`CREATE TABLE IF NOT EXISTS device_moves (
    ...
    d1.prepare("CREATE INDEX IF NOT EXISTS device_moves_expires_idx ON device_moves(expires_at)"),
  ...
  const missingEventColumns = [
    ...
    ["reveal_pickers", "ALTER TABLE events ADD COLUMN reveal_pickers INTEGER NOT NULL DEFAULT 0"],
