...
export default function PartyRoom({ code }: { code: string }) {
  ...
  const myOwnSongPlaying = Boolean(party?.currentTrack) && (party?.mySongs ?? []).some((song) => song.status === "playing");
  const songStartedAt = party?.currentTrackStartedAt ? new Date(party.currentTrackStartedAt).getTime() : Number.NaN;
  const warmingUp = Boolean(party?.currentTrack) && Number.isFinite(songStartedAt) && clock - songStartedAt < REACTION_GRACE_MS;
  const warmUpSecondsLeft = warmingUp ? Math.max(1, Math.ceil((REACTION_GRACE_MS - (clock - songStartedAt)) / 1_000)) : 0;
  ...
  async function react(kind: "up" | "down") {
    if (busy || myReaction || !party?.currentTrack || myOwnSongPlaying) return;
    if (warmingUp) { setNotice(`🎬 This song just started. Judge it in ${warmUpSecondsLeft}s.`); return; }
    setBusy(true);
    const intendedTrackId = party.currentTrack.id;
    const boost = kind === "up" && boostArmed;
    flyAway(kind === "up" ? (boost ? "⚡" : "🙌") : "👻", kind === "up" ? 28 : 72);
    buzz(kind === "up" ? (boost ? [30, 40, 60] : 30) : [20, 30, 20]);
    try {
      const data = await postAction({ action: "react", kind, boost, trackId: intendedTrackId });
      setParty((current) => ({ ...data.party, activity: current?.activity ?? [] }));
      setBoostArmed(false);
  ...
  return (
    <main className="party-shell">
      ...
      {party && ended && <section className="ended-banner"><strong>🏁 THAT’S A WRAP.</strong><span>🏆 No more votes. Boos are unmasked below. Bragging may continue indefinitely.</span><a className="recap-share hall-link" href={`/e/${code}/recap${isDevelopmentHost(window.location.hostname) && personaFromSearch(window.location.search) ? `?persona=${encodeURIComponent(personaFromSearch(window.location.search))}` : ""}`}>🏛️ Open the Hall of Fame →</a><button type="button" className="recap-share" onClick={() => void shareRecap()} disabled={recapBusy}>{recapBusy ? "📸 Drawing…" : "📸 Share the recap card"}</button></section>}
