        ...
        <section className={`now-playing ${!party.currentTrack ? "empty-player" : ""}`} aria-labelledby="playing-title">
          ...
          {party.currentTrack ? <>
            ...
            {!ended && <div className="reaction-panel"><div className="reaction-actions">
              <button className={`reaction-button cheer ${myReaction === "up" ? "selected" : ""}`} type="button" onClick={() => void react("up")} disabled={busy || Boolean(myReaction) || myOwnSongPlaying} aria-pressed={myReaction === "up"}><span className="reaction-icon" aria-hidden="true">🙌</span><span><strong>CHEER</strong><small>{myOwnSongPlaying ? "no self-cheering" : myReaction === "up" ? "locked in" : myReaction ? "vote already locked" : "make some noise"}</small></span>{myReaction === "up" && <b className="your-vote-badge">✓ YOUR VOTE</b>}</button>
              <button className={`reaction-button boo ${myReaction === "down" ? "selected" : ""}`} type="button" onClick={() => void react("down")} disabled={busy || Boolean(myReaction) || myOwnSongPlaying} aria-pressed={myReaction === "down"}><span className="reaction-icon" aria-hidden="true">👻</span><span><strong>BOO</strong><small>{myOwnSongPlaying ? "your pick, your problem" : myReaction === "down" ? "locked anonymously" : myReaction ? "vote already locked" : `${boosToSkip} boos skip it`}</small></span>{myReaction === "down" && <b className="your-vote-badge">✓ YOUR VOTE</b>}</button>
