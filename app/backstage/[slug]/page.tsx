...
export default async function AdminPage({ params }: AdminPageProps) {
  ...
  // The secret path is checked before sign-in so a wrong guess looks exactly like any other missing page.
  if (!isAdminPath(slug)) notFound();
  const pagePath = `/backstage/${slug}`;
  const user = await requireChatGPTUser(pagePath);
  const access = adminAccessForEmail(user.email);
  if (!access.configured || !access.allowed) {
    return <main className="admin-denied"><span className="brand-mark">HM</span><p className="eyebrow">🔒 OWNER ACCESS ONLY</p><h1>{access.configured ? "This account isn’t on the list." : "The owner allowlist isn’t configured yet."}</h1><p>Signed in as <strong>{user.email}</strong>. {access.configured ? "Use the ChatGPT account approved by the site owner." : "Configure ADMIN_ALLOWED_EMAILS before opening the dashboard."}</p><div><a href={chatGPTSignOutPath(pagePath)}>Switch ChatGPT account →</a><a href="/">Back to HackMusic</a></div></main>;
  }
  return <AdminDashboard ownerEmail={user.email} signOutPath={chatGPTSignOutPath("/")} apiBase={`/api/backstage/${slug}`} />;
}
